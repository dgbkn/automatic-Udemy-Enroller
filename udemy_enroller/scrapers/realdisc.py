...
class AppRealDiscUdemyScraper(BaseScraper):
    """
    Contains any logic related to scraping of data from discudemy.com
    """

    DOMAIN = "https://app.real.discount/"

    def __init__(self, enabled, max_pages=None):
        super().__init__()
        self.scraper_name = "apprealdiscudemy"
        if not enabled:
    ...
    async def run(self) -> List:
        ...
        logger.info(
            f"Page: {self.current_page} of {self.last_page} scraped from app.real.discount"
        )
    ...
    async def get_links(self) -> List:
        ...
        discudemy_links = []
        self.current_page += 1
        coupons_data = await get(f"{self.DOMAIN}/free-courses/?page={self.current_page}")
        soup = BeautifulSoup(coupons_data.decode("utf-8"), "html.parser")
        for course_card in soup.find_all("a", href=True):
          if(course_card["href"].find('offer') != -1):
            url_end = course_card["href"].split("/")[-1]
            discudemy_links.append(f"{self.DOMAIN}/offer/{url_end}")
    ...
    def _get_last_page(soup: BeautifulSoup) -> int:
        ...
        return max(
            [
                int(i.text)
                for i in soup.find("ul", class_="pagination").find_all("li")
                if i.text.isdigit()
            ]
        )
